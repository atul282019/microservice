...
import com.gov.nha.bis.npci.heartbit.service.HeartBitService;
...
public class HeartBitServiceImpl implements HeartBitService {

	

	private static final Logger logger = LoggerFactory.getLogger(HeartBitServiceImpl.class);
	...
	public String reqHbtForSignature(ReqHbt reqHbt) throws BisException {
		try {
			String xml = convertToXML(reqHbt).replace("xmlns=\"\"", "");
			xml = xml.replace("<ReqHbt xmlns=","<upi:ReqHbt xmlns:upi=");
			xml = xml.replace("</ReqHbt>","</upi:ReqHbt>");
	...
	@Override
	public RespHbt convertToHeartBeatObject(String hbtResXML) throws BisException {
		try {
			logger.info("convertToObject-------"+hbtResXML);	
			Document inputDocument = convertXmlToDocument(hbtResXML);
			removeSignatureElement(inputDocument);
			return (RespHbt)this.t.unmarshal(inputDocument.getDocumentElement(), RespHbt.class);
		} catch (BisException e) {
			logger.error("Error while unmarshalling the auth response", (Throwable)e);
			throw new BisException("Error while unmarshalling the auth response", e);
		} catch (Exception e) {
	...
	@Override
	public Document convertXmlToDocument(String authResXML) throws SAXException, IOException, ParserConfigurationException {
	...
	public Ack authenticate(String reqHbtForSignatureXml,String urlTxn,String uuid) throws BisException {
		try {
			customLogGenerator.generateLogFile(postUrl(urlTxn), uuid, "HBT");
			String authResXML = HttpClient.post(postUrl(urlTxn), reqHbtForSignatureXml, null,null);
			customLogGenerator.generateLogFile(authResXML, uuid, "HBT");			
			return convertToObject(authResXML);
		} catch (Exception e) {
			StringWriter sw = new StringWriter();
			PrintWriter pw = new PrintWriter(sw);
			e.printStackTrace(pw);
			customLogGenerator.generateLogFile(sw.toString(), uuid, "HBT");
			throw new BisException("Error while authenticating auth", e);
		} 
	}

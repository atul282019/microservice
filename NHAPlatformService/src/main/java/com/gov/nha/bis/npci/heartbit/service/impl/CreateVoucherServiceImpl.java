...
import com.gov.nha.bis.npci.heartbit.service.CreateVoucherService;
...
public class CreateVoucherServiceImpl implements CreateVoucherService {
	...
	private static final Logger logger = LoggerFactory.getLogger(CreateVoucherServiceImpl.class);
	...
	public String reqHbtForSignature(ReqVoucher reqvou) throws BisException {
		try {
			...
			xml = xml.replace("<ReqVoucher xmlns=","<upi:ReqVoucher xmlns:upi=");
			xml = xml.replace("</ReqVoucher>","</upi:ReqVoucher>");
	...
	@Override
	public RespVoucher convertToCreateResponseObject(String hbtResXML) throws BisException {
		try {
			logger.info("convertToObject-------"+hbtResXML);	
			Document inputDocument = convertXmlToDocument(hbtResXML);
			removeSignatureElement(inputDocument);
			return (RespVoucher)this.t.unmarshal(inputDocument.getDocumentElement(), RespVoucher.class);
		} catch (BisException e) {
			logger.error("Error while unmarshalling the auth response", (Throwable)e);
			throw new BisException("Error while unmarshalling the auth response", e);
		} catch (Exception e) {
	...
	public Ack authenticate(String reqHbtForSignatureXml,String urlTxn,String uuid) throws BisException {
		try {
			customLogGenerator.generateLogFile("Request URL - "+postUrl(urlTxn), uuid, "VOU_CR");
			String authResXML = HttpClient.post(postUrl(urlTxn), reqHbtForSignatureXml, null,null);
			customLogGenerator.generateLogFile("Response From NPCI - "+authResXML, uuid, "VOU_CR");
			return convertToObject(authResXML);
		} catch (Exception e) {
			StringWriter sw = new StringWriter();
			PrintWriter pw = new PrintWriter(sw);
			e.printStackTrace(pw);
			customLogGenerator.generateLogFile(sw.toString(), uuid, "VOU_CR");
			throw new BisException("Error while authenticating auth", e);
		} 
	}
